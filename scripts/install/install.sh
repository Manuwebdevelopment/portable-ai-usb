#!/bin/bash
# Portable AI USB Installer for Ubuntu/Debian
...
log_info "Portable AI USB Installer for Ubuntu/Debian"
...
install_dependencies() {
    ...
    if command -v apt-get &>/dev/null; then
        apt-get update -qq || {
            log_warn "apt-get update failed, trying apt..."
            apt update -qq
        }
    elif command -v dnf &>/dev/null; then
        dnf makecache -y -qq || dnf clean all && dnf makecache -y
    fi
...
install_ollama() {
    ...
    # Download Ollama
    curl -fsSL \
        https://ollama.com/download/ollama-$(($(uname -m | grep -q "arm" && echo "arm64" || echo "amd64"))-linux.tar.gz \
        -o /tmp/ollama-linux.tar.gz || {
        # Fallback for arm64 detection
        curl -fsSL \
            https://ollama.com/download/ollama-$(uname -m)-linux.tar.gz \
            -o /tmp/ollama-linux.tar.gz
    }
    
    if [[ $? -ne 0 ]]; then
        log_warn "Direct download failed, trying official location..."
        curl -fsSL \
            https://ollama.com/download/ollama-$(uname -m)-linux.tar.gz \
            -o /tmp/ollama-linux.tar.gz
    fi
    ...
    # Extract Ollama
    tar -xzf /tmp/ollama-linux.tar.gz -C /tmp
...
EOF
    ...
    if [[ ! -f /etc/ollama/ollama.conf ]]; then
        mkdir -p /etc/ollama
...
EOF
    ...
    # Create service unit if systemd exists
...
[Service]
Type=simple
# Don't use systemd-managed user, run as root or create ollama user
ExecStart=/usr/bin/ollama serve
Restart=on-failure
RestartSec=5s
TimeoutStopSec=30s
# Drop capabilities and run with reduced privileges
...
OLLAMA_MODELS=$OLLAMA_DATA_DIR
OLLAMA_CONFIG=$CONFIG_DIR
EOF
...
Ollama has been successfully installed!
